...
Le build `make hns -j4` passe. La référence automatisée passe avec `Native` 3/3, `Audio` 6/6, `Settings` 5/5, `Family` 27/27, `Evolution requirements` 4/4, `Level cap` 9/9, `Training NPC` 7/7 et `Family Safari` 4/4. Le bloc rencontres ajoute `Tactica encounters` 2/2 et un validateur qui compare les 405 tables compilées à la source canonique. Les validateurs statiques `validate_tactica_spec.py` et `validate_family_remix.py` passent également. Ces résultats établissent l'absence de régression de compilation sur la base importée ; ils ne valident pas les contrats obsolètes explicitement relevés ci-dessous. En particulier, le test `Level cap` attend encore l'enchaînement Rocket `+2`.
