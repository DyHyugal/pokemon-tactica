...
| Wiki FR/EN | `[PARTIEL]` | `feature/wiki` @ `38e99bb1` : pages cœur FR/EN reconstruites ; Localisations générées depuis 405 tables standard + 57 spéciales ; Pokédex séparé et noms FR relocalisés ; 26 modifications + 15 learnsets alignés ; 151 slots de boss publiés ; palette contrôlée contre `option_menu_text_custom.pal` / menus ; QA liens locaux des 15 pages cœur = 0 cassé. Reste avant `[DONE]` : fusion `integration/v1` puis revue visuelle navigateur/mobile. |
