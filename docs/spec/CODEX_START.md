...
3. Exécute seulement les vérifications appropriées pour établir la baseline. Ne déclare pas validé un test mGBA que tu n'as pas joué. Signale les écarts et propose l'ordre des PR. Les blocs encounters, balance, boutiques et Rocket ont maintenant des sources et tests automatisés. Le câblage dynamique des six équipes du rival est réalisé sur `feature/rival-v1` avec table générée et tests ; après intégration, la priorité est le dataset `earliest_access_cap`, puis la revue FR/EN et la recette ROM.
...
- Le rival tire catégorie puis starter, conserve le résultat et développe un thème de combat correspondant. Les six rosters et leurs phases de `data/spec/rival.json` sont générés dans `src/data/tactica_rival.h` puis résolus au chargement des combats existants. Toute évolution de ce contrat doit régénérer la table et conserver les tests ; la validation en ROM reste requise.
