...
3. Exécute seulement les vérifications appropriées pour établir la baseline. Ne déclare pas validé un test mGBA que tu n'as pas joué. Signale les écarts et propose l'ordre des PR. Les blocs encounters, balance, boutiques et Rocket ont maintenant des sources et tests automatisés sur une branche de feature ; la priorité restante est le câblage des six équipes du rival, le contrôle des évolutions/capacités au niveau réel et le dataset `earliest_access_cap` ; revue FR/EN et recette ROM suivent les mêmes JSON.
...
- Le rival tire catégorie puis starter, conserve le résultat et développe un thème de combat correspondant. Les six rosters et leurs phases sont rédigés dans `data/spec/rival.json` ; il faut encore les relier aux combats existants, résoudre les évolutions et capacités légales, puis les valider en ROM.
