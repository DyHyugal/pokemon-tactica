...
function addNavLink(){document.querySelectorAll(".nav").forEach(nav=>{if(nav.querySelector('a[href="Routes-et-Villes.html"],a[href="Routes-and-Cities.html"]'))return;const loc=nav.querySelector('a[href="Localisations.html"],a[href="Locations.html"]');if(!loc)return;const a=document.createElement("a");a.href=fr?"Routes-et-Villes.html":"Routes-and-Cities.html";a.textContent=fr?"Routes et Villes":"Routes & Cities";loc.insertAdjacentElement("afterend",a)})}
...
addNavLink();document.getElementById("place-search").addEventListener("input",render);document.querySelectorAll(".place-tab").forEach(b=>b.addEventListener("click",()=>{document.querySelectorAll(".place-tab").forEach(x=>x.classList.remove("active"));b.classList.add("active");active=b.dataset.kind;render()}));document.querySelector(".place-close").addEventListener("click",()=>dialog.close());dialog.addEventListener("click",e=>{if(e.target===dialog)dialog.close()});render();
})();