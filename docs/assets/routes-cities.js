...
async function encounters(x){if(!encDoc){const url=fr?"Localisations.html":"Locations.html";try{const html=await fetch(url).then(r=>r.text());encDoc=new DOMParser().parseFromString(html,"text/html")}catch(e){return ""}}const aliases=[x[0].toLowerCase(),x[1].toLowerCase()];const rows=[...encDoc.querySelectorAll("#loc tbody tr")].filter(r=>aliases.includes((r.children[0]?.textContent||"").trim().toLowerCase()));if(!rows.length)return '<p class="muted">'+(fr?"Aucune table sauvage publiée pour cette zone.":"No published wild table for this area.")+'</p>';return '<div class="route-encounters">'+rows.map(r=>'<div><strong>'+r.children[1].textContent+'</strong><span>'+r.children[2].textContent+' · '+r.children[3].textContent+'</span><small>'+r.children[4].textContent+'</small></div>').join("")+'</div>'}
async function open(x){content.innerHTML='<span class="eyebrow">'+(x[2]==="city"?(fr?"VILLE":"CITY"):(fr?"ROUTE":"ROUTE"))+'</span><h1>'+display(x)+'</h1>'+(fr&&x[0]!==x[1]?'<p class="muted">Nom interne/EN : '+x[0]+'</p>':'')+section(names.story,x[4])+section(names.interest,x[3])+section(names.loot,x[5])+section(names.special,x[6])+'<section><h2>'+names.enc+'</h2><div id="route-enc-box"><p class="muted">…</p></div><a class="detail-location-link" href="'+(fr?"Localisations.html?q=":"Locations.html?q=")+encodeURIComponent(display(x))+'">'+names.see+' →</a></section>';if(!dialog.open)dialog.showModal();document.getElementById("route-enc-box").innerHTML=await encounters(x)}
